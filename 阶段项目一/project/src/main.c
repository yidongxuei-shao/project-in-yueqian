...
int main(int argc, char *buf[])
{
    ...
    while(1)
    {
        ...
        if(x == 0 && y == 0)    /* 用来反映用户是否触摸屏幕，有反应就进入主界面*/
        {
            /* 捕获触摸坐标 */
            TS_GetVal(ts_fd, &x, &y);
        }
...
get_ts_user_msg_show:
        ...
        else if(flag == 2)
        {
            ...
            {
                /* 播放GIF动图，速度快些 */
                GIF_Show(GIF_CHARGE, lcd_info, FAST_CHARGE, 0x000000, 150,0);
            }
            else if(flag == 2) /* 普通充电 */
            {
                /* 播放GIF动图，速度慢些 */
                GIF_Show(GIF_CHARGE, lcd_info, ORDIN_CHARGE, 0x000000, 150 ,0);
            }
            else    /* 返回值为 -1 */
            {
...
user_log_off_again:
        ...
        /* 获取充电时长与充电金额 */
        float time_char = get_amount_spent(Pmanage, account_name);
        printf("charge = %f\r\n",time_char);
#if 0
    ...
    printf("time end = %d ,%d, %d \r\n", data->time_end->hour,data->time_end->min,data->time_end->sec);
        ...
        /* GIF显示完后，显示充电时长，充电金额 */
        flag = show_use_time_mon(time_char, data->balance, lcd_info, ts_fd);  /* 界面显示金额和充电分钟 */
...
get_again:
    TS_GetVal(ts_fd, &x, &y);
    if(x > 179 && x < 748 && y > 83 && y < 169)
    {
        /* 呼出输出键盘 */
        read_JPEG_file(0,480-310,PIC_KEY_PAD,lcd_info);
    }
...
/*
    ...
    函数版本：V1.0
*/
float get_amount_spent(Hnode_t* Pmanage, const char* account_name)
{
    /* 再次获取时间 */
    get_time_t* time = get_time();
    ...
    */
    ...
    charge_sec = (hour * 60 + min * 60 + sec) * discount * CHARGE_MONEY;
...
/*
    ...
    函数版本：V1.0
*/
int show_use_time_mon(float time, int remain_money ,LcdInfo_t* lcd_info, int ts_fd)
{
    /* 加载图片，用户结算界面 */
    ...
    sprintf(buf, "%.0f", time * CHARGE_MONEY);
    len = strlen(buf);
    /* 显示金额 */
    for(i = 0;i < len;)
    {
        int number =   buf[i] - '0';
        show_number(lcd_info, &i, number, PIC_NUMBER,CHARGE_POS_X,185);
    }
    printf("momory = %d\t\n", number);
    ...
    {
        int number =   buf[i] - '0';
        show_number(lcd_info, &i, number, PIC_NUMBER,CHARGE_POS_X,260);
    }

retry:
    TS_GetVal(ts_fd, &x, &y);
    /* 确认键 */
    if(x > 331 && x < 447 && y > 322 && y < 387)
    {
        return 1;
    }
    else
...
inval:  /* 无动作 */       
        ...
        set_postion_keypad();
        /* 呼出输入键盘 */
        int flag =  show_input(ts_fd,lcd_info, PIC_RECHAGER, buf, CHARGE_MAX, NUMBER_POS_STA_X, NUMBER_POS_STA_Y);
        if(flag == -1)
            return -1;
